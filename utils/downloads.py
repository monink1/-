...
def safe_download(file, url, url2=None, min_bytes=1E0, error_msg=''):
    ...
    try:  # url1
        ...
        if not file.exists() or file.stat().st_size < min_bytes:  # check
            #   python3.8以下unlink没有参数，可以ctrl + B查看，运行到此函数可能会报错
            file.unlink(missing_ok=True)  # remove partial downloads
...
def gdrive_download(id='16TiPfZj7htmTyhntwcZyEEAejOUxuT6m', file='tmp.zip'):
    # Downloads a file from Google Drive. from yolov5.utils_cls.downloads import *; gdrive_download()
    t = time.time()
    file = Path(file)
...
# Google utils_cls: https://cloud.google.com/storage/docs/reference/libraries ----------------------------------------------
#
#
