...
class IOUloss(nn.Module):
    def __init__(self, reduction="none", loss_type="ciou"):
        ...
        self.eps = 1e-16
        self.iou_mean = 1.0
        self.momentum = 1 - pow(0.5, 1/7000)
    ...
    def forward(self, pred, target, xyxy=False):
        ...
        if self.loss_type == "iou":
            ...
            center_dis = (torch.pow(pred[:, 0] - target[:, 0], 2) + torch.pow(pred[:, 1] - target[:, 1],
                                                                              2))  # center diagonal squared
            dist = torch.exp(center_dis / convex_dis.detach())
            l_iou = 1 - iou
            l_wiou1 = dist * l_iou
            ...
            loss = l_wiou1 * (beta / alpha)
            self.iou_mean = (1 - self.momentum) * self.iou_mean + self.momentum * l_iou.detach().mean().item()
...
class simOTA(nn.Module):
    ...
    def forward(self, outputs_p, labels, imgs):
        ...
        # outputs_p: bs, 3, h, w, num+5
        # labels: 目标数， 6(图片序号， 类别， 坐标)
    ...
    def get_output_and_grid(self, output, k, stride, dtype):
        ...
        anchor_grid = (self.anchors[k].clone() * self.strides[k]) \
            .view((1, self.n_anchors, 1, 1, 2)).expand((1, self.n_anchors, hsize, wsize, 2)).reshape(
            (1, -1, 2)).float().type(dtype)
    ...
    def get_losses(
        ...
        nlabel = labels[:, 0].long().bincount(minlength=cls_preds.shape[0])
        torch.use_deterministic_algorithms(True)
        max_num = nlabel.max()
        labels_tmp = torch.zeros(bbox_preds.shape[0], max_num, 5)
...
class distill_simOTA(nn.Module):
    def __init__(
            self,
            num_classes,
            iou_type='ciou',
    ...
    def forward(self, outputs_p, outputs_t, labels, imgs):
        outputs = []
    ...
    def get_losses(
        ...
        bbox_preds_t = outputs_t[:, :, :4]  # [batch, n_anchors_all, 4]
        obj_preds_t = outputs_t[:, :, 4].unsqueeze(-1)  # [batch, n_anchors_all, 1]
        cls_preds_t = outputs_t[:, :, 5:]  # [batch, n_anchors_all, n_cls]
        ...
        for batch_idx in range(outputs.shape[0]):
            ...
            if num_gt == 0:
                ...
                gt_bboxes_per_image = labels[batch_idx, :num_gt, 1:5]
                gt_classes = labels[batch_idx, :num_gt, 0]
                bboxes_preds_per_image = bbox_preds_t[batch_idx]
